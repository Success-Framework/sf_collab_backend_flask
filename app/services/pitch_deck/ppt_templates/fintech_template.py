"""
fintech_template.py
Theme: Forest Trust — Dark Green (#1B3A2D) + Gold (#C9A84C) + Slate (#2E4057)
Visual motif: Left accent bar (gold) + right-side slate panel on content slides
"""
...
BG_DARK   = "0F2318"   # very dark green
BG_MID    = "1B3A2D"   # forest green
BG_LIGHT  = "F2F7F4"   # pale mint
ACCENT    = "C9A84C"   # gold
ACCENT2   = "2E4057"   # slate
TEXT_LITE = "FFFFFF"
TEXT_DARK = "0F2318"
TEXT_MID  = "2C5440"
CARD_BG   = "D6EDE0"
...
def _dark_header(slide, title, num):
    ...
    add_slide_number(slide, num, TOTAL_SLIDES, "7A9A80")
...
def _slide_cover(prs, content):
    ...
    # Right gold panel
    add_rect(slide, left=Inches(9.8), top=Inches(0),
             width=Inches(3.53), height=SLIDE_HEIGHT, fill_hex=ACCENT)

    # Decorative lines on gold panel
    for i in range(5):
        add_rect(slide, left=Inches(9.9), top=Inches(0.8 + i * 1.2),
                 width=Inches(3.3), height=Inches(0.03), fill_hex=BG_MID)

    add_textbox(slide, content.get("company_name",""),
        left=Inches(0.7), top=Inches(1.6),
        width=Inches(8.8), height=Inches(1.4),
    ...
    add_rect(slide, left=Inches(0.7), top=Inches(3.95),
             width=Inches(3.5), height=Inches(0.04), fill_hex=ACCENT)
    ...
    add_textbox(slide, content.get("subtitle","Investor Presentation"),
        ...
        color_hex="A8C4A2", align=PP_ALIGN.LEFT)
...
def _slide_problem(prs, content):
    ...
    for i, b in enumerate(bullets):
        ...
        add_rect(slide, left=cx, top=Inches(2.15), width=card_w, height=Inches(0.35), fill_hex=ACCENT)
        add_textbox(slide, f"0{i+1}",
            left=cx+Inches(0.1), top=Inches(2.16),
            width=Inches(1.0), height=Inches(0.35),
            font_name=TITLE_FONT, font_size=13, bold=True,
            color_hex=BG_DARK, align=PP_ALIGN.LEFT)
        add_textbox(slide, b,
            left=cx+Inches(0.15), top=Inches(2.65),
            width=card_w-Inches(0.3), height=Inches(3.0),
            font_name=BODY_FONT, font_size=13,
...
def _slide_solution(prs, content):
    slide = add_blank_slide(prs)
    fill_slide_background(slide, BG_MID)
    ...
    add_rect(slide, left=Inches(7.1), top=Inches(2.0),
             width=Inches(5.9), height=Inches(4.8), fill_hex=BG_DARK)
    add_textbox(slide, "$",
    ...
    add_textbox(slide, "Trusted & Compliant",
...
def _slide_market(prs, content):
    ...
    for i, (lbl, key, abbr) in enumerate(zip(labels, keys, abbrv)):
...
def _slide_product(prs, content):
    ...
    for i, feat in enumerate(features):
        ...
        add_rect(slide, left=cx, top=Inches(2.0), width=feat_w, height=Inches(0.5), fill_hex=ACCENT)
        ...
        add_textbox(slide, feat.get("description",""),
            left=cx+Inches(0.15), top=Inches(2.65),
            width=feat_w-Inches(0.3), height=Inches(2.8),
            font_name=BODY_FONT, font_size=13,
            color_hex=TEXT_LITE, align=PP_ALIGN.LEFT)
...
def _slide_traction(prs, content):
    ...
    add_rect(slide, left=Inches(0.5), top=Inches(4.52),
             width=Inches(12.5), height=Inches(0.04), fill_hex=ACCENT)
    ...
    for i, ms in enumerate(milestones):
        ...
        add_circle(slide, left=cx+m_w2/2-Inches(0.1), top=Inches(4.42),
                   diameter=Inches(0.2), fill_hex=ACCENT)
...
def _slide_team(prs, content):
    ...
    for i, m in enumerate(members):
        ...
        add_rect(slide, left=cx, top=Inches(2.0), width=card_w, height=Inches(0.35), fill_hex=ACCENT)
        add_circle(slide, left=cx+card_w/2-Inches(0.5), top=Inches(2.5),
                   diameter=Inches(1.0), fill_hex=BG_DARK)
        add_textbox(slide, m.get("name","?")[:1].upper(),
            left=cx+card_w/2-Inches(0.5), top=Inches(2.55),
            width=Inches(1.0), height=Inches(0.9),
            font_name=TITLE_FONT, font_size=22, bold=True,
            color_hex=ACCENT, align=PP_ALIGN.CENTER)
        add_textbox(slide, m.get("name",""),
            left=cx+Inches(0.1), top=Inches(3.65),
            width=card_w-Inches(0.2), height=Inches(0.45),
            font_name=TITLE_FONT, font_size=13, bold=True,
            color_hex=TEXT_LITE, align=PP_ALIGN.CENTER)
        add_textbox(slide, m.get("role",""),
            left=cx+Inches(0.1), top=Inches(4.12),
            width=card_w-Inches(0.2), height=Inches(0.35),
            font_name=BODY_FONT, font_size=11,
            color_hex=ACCENT, align=PP_ALIGN.CENTER)
        add_textbox(slide, m.get("highlight",""),
            left=cx+Inches(0.1), top=Inches(4.6),
            width=card_w-Inches(0.2), height=Inches(1.1),
            font_name=BODY_FONT, font_size=11,
            color_hex=CARD_BG, align=PP_ALIGN.CENTER)
...
def build_fintech_deck(content: dict):
